...
- [AB-EV-052 — C45G deep country and visit editor localization](regression/ab-ev-052-c45g-deep-country-visit-editor-localization.md)
...
- [AB-EV-057 — C45L password-reset action localization and release qualification](regression/ab-ev-057-c45l-password-reset-action-localization.md)
...
AB-EV-039 records per-memory privacy and sanitised public projection. AB-EV-040 records manual memory presentation order plus the correctly classified Rules-parity blocker. AB-EV-041 records public-memory access from earned flags, owner/anonymous public-source parity, exact visit labels and the final Profile sort-layout correction. AB-EV-042 records editable visit names with stable identity, explicit Save and sanitised public labels. AB-EV-043 records the repository-wide visual identity alignment and semantic-colour preservation. AB-EV-044 records bounded visit-photo storage/quota, real Firebase Admin/runtime qualification, public photo projection, adjacent profile-write regressions and final Production approval. AB-EV-045 records the six-route public localisation foundation, root locale resolution, `[username]` route protection, Next.js security hardening and Production smoke. AB-EV-046 records translated public Home surfaces, desktop/mobile and authenticated-Home selector behaviour, locale preference persistence, server-correct document language, dynamic-rendering architecture review and final Production/mobile visual approval. AB-EV-052 records the localized deep country/visit editor, the preserved C35/C39/C40/C42/C44 domain contracts, additive stable error-code strategy, stateful Emulator regression and Production technical closure. AB-EV-053 records localized authenticated Badges/achievement presentation and Badge Unlock notification, all 31 stable achievement IDs, metadata/chronology/reconquest preservation, public-Profile isolation and safe Production technical smoke. AB-EV-054 records localized Profile Edit and embedded account/security presentation, scoped provider isolation, stable validation-code presentation, preserved persistence/security contracts, safe Production smoke and final Production visual approval. C41–C45I plus AB-EV-049 reached Production and final Test Lead approval. AB-EV-055 consolidates C45J six-locale canonical public Profile localization and its two real Production visual Product Defects: AB-DEF-019 (authenticated Header language fallback) and AB-DEF-020 (loading scrollbar layout shift). The record links each correction to its exact Product commit, READY Production deployment, proportionate Emulator regression and Test Lead visual sign-off (final FIX2 acceptance on 20 September 2026). The geographic display-name checkpoint remains separate and unimplemented.

AB-EV-052 also records a visual-QA test-data incident: a direct rich Emulator seed bypassed achievement-metadata reconciliation and was correctly rejected by the Product development assertion. Data-only reconciliation restored `/app`, `/badges` and the public Profile without Product changes. It is classified as a QA fixture defect, not a Product Defect or C45G regression. AB-EV-053 demonstrates the resulting standing control by requiring reconciled achievement metadata before C45H visual handoff.
...
The Production-validation cycles also found test-harness/environment defects involving target selection, authentication setup, stale accessible names, search assumptions, optimistic-state synchronisation, Rules parity, browser-session/origin assumptions and incomplete derived-state fixtures. These are QA-process evidence rather than Product Defects unless the product itself fails under a valid environment and valid Product data.

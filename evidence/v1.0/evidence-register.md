...
|---|---|---|
| QR-01 | Remains `Current gap`; map persistence path is scoped as mitigated | AB-EV-008 proves confirmed-cache separation, rollback, retry and affected-area regression for map status/visit flows; equivalent coverage for all product persistence flows and final Production smoke remain pending. |
...
| QR-05 | `Current gap` → `Regression risk`, with a reformulated description | The active legacy non-visit memory editor persisted text on each `onChange`. Commit `e73fad9` replaced that path with local draft state and explicit Save while preserving the internal `generalNote` model, visit integrity and Production-approved behaviour. |
| QR-06 | Still pending | No final evidence package or status decision is available yet. |

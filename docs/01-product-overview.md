...
Public achievement metadata contains only `unlockedAt` and `sequence`. C45H/AB-EV-053 localizes achievement presentation by stable achievement ID without changing or expanding that public metadata projection. C45J now localizes public Profile achievement titles/descriptions by reusing the existing C45H stable-ID presentation catalogue. Before C45J the public Profile deliberately stayed Portuguese; C45J changed only the presentation boundary, not the approved public achievement metadata.
...
- Remaining localisation beyond completed C45A–C45J (+ FIX1/FIX2): password-reset action presentation, the future authenticated language selector and canonical country/continent display-name mapping. Geographic names are a **new presentation-only checkpoint** and must not mutate stored identifiers or user-authored values;
